...
module.exports = class Strategy_V1 {
    static apply() {
        if (gb.currentBuyers > gb.currentSellers && gb.lastAction !== conf.BUY && gb.lastSellPrice >= gb.bidsAverage && gb.lastOrderWasFilled) {
            trader.placeBuyOrder(gb.bidsAverage);
            gb.lastOrderWasFilled = false;
        }
        if (gb.currentSellers > gb.currentBuyers && gb.lastAction !== conf.SELL && gb.asksAverage > gb.lastBuyPrice && gb.lastOrderWasFilled) {
