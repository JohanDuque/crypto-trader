...
class Strategy_V8 {
    ...
    aboutToBuyWhileMarketDropping() {
        if (analyzer.isMarketGoingFromUpToDown()) {
            Logger.log(1, "Market is GoingFromUpToDOWN I'm waiting to buy at lower price");
            return;
        }

        if (gb.currentSellSpeed > conf.tradeHistorySize - 2) {
    ...
    aboutToSellWhileMarketGrowing() {
        if (analyzer.isMarketGoingFromDownToUp()) {
            Logger.log(1, "Market is GoingFromDownToUP I'll wait to SELL at higher price");
            return;
        }
    ...
    buyOrderPlacedWhileMarketGrowing() {//TODO here
...
module.exports = new Strategy_V8();